...
const API_BASE_URL = import.meta.env.REACT_APP_API_BASE_URL || "https://securelink-backend.onrender.com";
...
// Fetch Analytics
export const fetchAnalytics = async (_id) => {
	const authToken = localStorage.getItem("token");
	if (!authToken) {
		throw new Error("No token found. Please log in again.");
	}
	...
	try {
		const response = await apiClient.get(`/analytics/${_id}`, {
			headers: {
				Authorization: `Bearer ${authToken}`,
			},
		});
